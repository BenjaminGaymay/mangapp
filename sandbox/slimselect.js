(function webpackUniversalModuleDefinition(root, factory) {
	...
	else if (typeof exports === 'object') exports['SlimSelect'] = factory();
	else root['SlimSelect'] = factory();
})(window, function () {
			...
			/***/ function (module, exports, __webpack_require__) {
				...
				var Data = (function () {
					...
					Data.prototype.pullOptionData = function (option) {
						return {
							id:
								(option.dataset ? option.dataset.id : false) ||
			...
			/***/ function (module, exports, __webpack_require__) {
				...
				var SlimSelect = (function () {
					function SlimSelect(info) {
						...
						// info : {"select":"#chapters","searchText":"Aucun résultat !"}
						var _this = this;
						this.ajax = null;
						this.addable = null;
							...
							helper_1.debounce(function (e) {
								if (_this.data.contentOpen && _this.config.showContent === 'auto') {
									if (
										helper_1.putContent(
											_this.slim.content,
											_this.data.contentPosition,
											_this.data.contentOpen
										) === 'above'
									) {
				...
				var Slim = (function () {
					...
					Slim.prototype.placeholder = function () {
						...
						if (selected === null || (selected && selected.placeholder)) {
							...
							if (selected) {
								selectedValue =
									selected.innerHTML && this.main.config.valuesUseText !== true
										? selected.innerHTML
										: selected.text;
							}
							if (this.singleSelected) {
					...
					Slim.prototype.valueDiv = function (option) {
						...
						text.innerHTML =
							option.innerHTML && this.main.config.valuesUseText !== true
								? option.innerHTML
								: option.text;
								...
								if (_this.main.addable) {
									...
									if (typeof addableValue == 'object') {
										...
										if (validValue) {
											...
											addableValueStr_1 = addableValue.value
												? addableValue.value
												: addableValue.text;
										}
									} else {
										_this.main.addData(
											_this.main.data.newOption({
												text: addableValue,
												value: addableValue
											})
										);
						...
						if (prev === null) {
							...
							if (parent.classList.contains(this.main.config.optgroup)) {
								if (parent.previousSibling) {
									var prevNodes = parent.previousSibling.querySelectorAll(
										'.' + this.main.config.option + ':not(.' + this.main.config.disabled + ')'
									);
									if (prevNodes.length) {
					...
					Slim.prototype.option = function (data) {
						...
						if (
							...
							option.innerHTML = helper_1.highlight(
								data.innerHTML,
						...
						option.addEventListener('click', function (e) {
							...
							e.stopPropagation();
							if (
								master.main.config.limit &&
								Array.isArray(selected) &&
								master.main.config.limit <= selected.length
							) {
								return;
							}
							var element = this;
							...
							if (master.main.beforeOnChange) {
								...
								var objectInfo = JSON.parse(
									JSON.stringify(master.main.data.getObjectFromData(elementID))
								);
